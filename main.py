...
os.environ['Video center'] = '1'
info = pygame.display.Info()

font = pygame.font.Font('settings/Terserah.ttf', 48)
medium_font = pygame.font.Font('settings/Terserah.ttf', 28)
small_font = pygame.font.Font('settings/Terserah.ttf', 16)
real_small_font = pygame.font.Font('settings/Terserah.ttf', 10)
FPS = 60
timer = pygame.time.Clock()
WIDTH = 52 * 35
HEIGHT = 400
SCREEN_WIDTH, SCREEN_HEIGHT = info.current_w, info.current_h
screen = pygame.display.set_mode([SCREEN_HEIGHT - 10, SCREEN_HEIGHT - 50], pygame.RESIZABLE)
white_sounds = []
black_sounds = []
active_whites = []
active_blacks = []
...
for i in range(len(white_notes)):
    white_sounds.append(mixer.Sound(f'settings\\notes\\{white_notes[i]}.wav'))

for i in range(len(black_notes)):
    black_sounds.append(mixer.Sound(f'settings\\notes\\{black_notes[i]}.wav'))
...
def draw_title_bar():
    ...
    img = pygame.transform.scale(pygame.image.load('img/813hIf-biqL.png'), [150, 150])
    screen.blit(img, (-20, -30))
    title_text = font.render('Just feel it!', True, 'white')
    screen.blit(title_text, (140, 25))
    title_text = font.render('Take an inspiration!', True, 'black')
